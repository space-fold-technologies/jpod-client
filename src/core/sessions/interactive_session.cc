...
namespace core::operations 
{
  ...
  interactive_session::interactive_session(interactive_session_listener &listener) : 
  session(), 
  listener(listener)
  {
  }
  void interactive_session::on_data(response::operation operation, const std::vector<uint8_t> &content)
  {
    ...
    {
      listener.on_finish(false, std::string(content.begin(), content.end()));
      disconnect();
      break;
    }
    default:
  ...
  void interactive_session::on_open() 
  { 
    listener.on_start(false); 
  }
